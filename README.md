...
`docker-compose.yml` 기본 설정으로 `8000` 포트가 노출됩니다. 헬스체크가 활성화되어 있어 `/api/channels` 응답 기준으로 컨테이너 상태를 확인합니다.
...
```bash
# DB 모드로 배포 (SQLite PVC 5Gi)
helm install yt-agent charts/youtube-agent \
  --set secrets.youtubeApiKey=AIza... \
  --set secrets.anthropicApiKey=sk-ant-...

# 파일 모드로 배포 (DB 없이 config.json + 파일 요약)
helm install yt-agent charts/youtube-agent \
  --set storage.mode=file \
  --set secrets.youtubeApiKey=AIza...

# Ingress 활성화 (nginx, WebSocket timeout 자동 설정)
helm upgrade yt-agent charts/youtube-agent \
  --set ingress.enabled=true \
  --set ingress.host=agent.example.com \
  --set ingress.tls=true \
  --set ingress.tlsSecretName=agent-tls
```
...
| 리소스 | 설명 |
|---|---|
| Deployment | Recreate 전략 (RWO PVC 호환), non-root 실행 |
| Service | ClusterIP, 포트 8000 |
| Ingress | WebSocket proxy-read-timeout 3600s |
| PVC | data 5Gi / reports 2Gi (StorageClass 지정 가능) |
| ConfigMap | 비민감 환경변수 (폴링 간격, 로그 레벨 등) |
| Secret | API 키 (YouTube, Anthropic, Slack, Notion) |
...
│       ├── Chart.yaml
│       ├── values.yaml
│       └── templates/             # Deployment, Service, Ingress, PVC, Secret, ConfigMap
├── static/
...
└── src/
    ...
    ├── storage/
    │   ├── database.py            # SQLite 스키마 & CRUD (DB 모드)
    │   ├── store.py               # 스토리지 추상화 레이어 (DB ↔ 파일 라우팅)
    │   ├── config_store.py        # 파일 모드 설정 (data/config.json)
    │   └── summary_store.py       # 파일 모드 요약 (data/summaries/{채널}/{날짜}/)
    └── notifiers/
        ├── slack_notifier.py
        └── notion_notifier.py
```
