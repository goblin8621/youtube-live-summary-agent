...
## Phase 15 · 파일 모드 완전 지원 (채널·영상 기준점 추상화)

**배경:** Phase 14에서 설정·요약은 파일 모드로 전환했지만, 채널 CRUD·last_video_id·라이브 세션 저장이 여전히 DB를 직접 참조해 파일 모드에서 크래시 발생.

### store.py 추가 메서드

```python
# 채널 관리
store.get_channels()           → DB: monitored_channels 테이블 / 파일: config.json["channels"]
store.add_channel(channel)     → 중복 시 False 반환
store.remove_channel(id)       → 제거된 title 반환, 없으면 None
store.load_db_channels()       → 폴링 루프 시작 시 채널 ID set 반환

# 영상 기준점 (신규 영상 중복 감지)
store.get_last_video_id(ch)    → DB: channel_last_video 테이블 / 파일: config.json["last_video"][ch]
store.set_last_video_id(ch, v) → 동일
```
...
add_channel(channel: dict) → bool      # 중복 체크 포함
remove_channel(channel_id) → str|None  # 제거된 title
```
채널 목록은 `config.json["channels"]` 배열로 저장, last_video_id는 `config.json["last_video"]` 딕셔너리로 저장.

### poller.py 변경

| 변경 전 | 변경 후 |
|---|---|
| `_load_db_channels()` — aiosqlite 직접 쿼리 | `store.load_db_channels()` |
| `db.get_last_video_id()` | `store.get_last_video_id()` |
| `db.set_last_video_id()` | `store.set_last_video_id()` |
| `db.upsert_session()` | `if store.is_db_mode(): db.upsert_session()` |
| `db.insert_chat_messages()` | 파일 모드에서 스킵 (메모리만) |
| `db.close_session()` / `db.get_session()` | 파일 모드에서 메모리 dict 생성으로 대체 |
| `db.insert_captions()` / `db.get_captions()` | 파일 모드에서 captions_raw 직접 사용 |

### app.py 변경

| 엔드포인트 | 변경 내용 |
|---|---|
| `GET /api/channels` | `store.get_channels()` |
| `POST /api/channels` | `store.add_channel()` |
| `DELETE /api/channels/{id}` | `store.remove_channel()` |
| `GET /api/live` | 파일 모드: `poller._active_sessions` 메모리 반환 |

### 파일 모드 아키텍처 (완성)

```
시작 시: agent.db 없음 → 파일 모드
  설정: data/config.json["ai"], ["youtube"]
  채널: data/config.json["channels"]
  영상 기준점: data/config.json["last_video"]
  요약: data/summaries/{channel_id}/{YYYY-MM-DD}/*.json
  라이브 세션: 메모리만 (재시작 시 초기화)
  10일 경과 요약 폴더: 자동 삭제
```

---

## Phase 16 · Dockerfile 개선 및 Kubernetes Helm chart

### Dockerfile (multi-stage)

```dockerfile
FROM python:3.12-slim AS builder
RUN pip install --prefix=/install -r requirements.txt   # 빌드 레이어 분리

FROM python:3.12-slim
RUN groupadd -r agent && useradd -r -g agent agent      # non-root 실행
COPY --from=builder /install /usr/local
USER agent
HEALTHCHECK --interval=30s --timeout=10s ...            # /api/channels 헬스체크
```

**개선 포인트:** 이미지 크기 감소 (빌드 의존성 제외), non-root 보안 강화, 헬스체크 내장.
...
ports:
  - "8000:8000"  # 명시적 포트 노출 추가
```

### Helm chart (`charts/youtube-agent/`)

```
Chart.yaml        appVersion: "1.0.0"
values.yaml
templates/
  _helpers.tpl    # fullname, labels, selectorLabels
  deployment.yaml # Recreate 전략 (RWO PVC 호환), securityContext non-root
  service.yaml    # ClusterIP :8000
  ingress.yaml    # WebSocket: proxy-read-timeout/send-timeout 3600s
  pvc.yaml        # data 5Gi + reports 2Gi (RWO)
  configmap.yaml  # POLL_INTERVAL_SECONDS, LOG_LEVEL 등 비민감 env
  secret.yaml     # YOUTUBE_API_KEY, ANTHROPIC_API_KEY, SLACK_BOT_TOKEN, NOTION_TOKEN
```
...
```yaml
storage:
  mode: db        # "db" | "file" → DB_PATH 환경변수로 변환
  data.size: 5Gi
  reports.size: 2Gi
...
ingress:
  enabled: false
  className: nginx
  host: youtube-agent.example.com
  tls: false
...
**설계 결정:**
- `strategy: Recreate` — SQLite는 단일 프로세스 접근이 안전하며 RWO PVC는 동시 마운트 불가
- Ingress에 WebSocket timeout 3600s 설정 — 라이브 방송 중 WebSocket 연결 유지 필수
- `fsGroup: 999` — PVC 마운트 시 non-root 사용자가 쓰기 가능하도록 설정
...
스토리지 추상화 (store.py)
  ├─ DB 모드  : 설정/채널/요약/기준점 → SQLite (app_settings, monitored_channels, summaries 테이블)
  └─ 파일 모드: 설정/채널/기준점 → data/config.json / 요약 → data/summaries/{채널}/{날짜}/
               라이브 세션·채팅·자막 → 메모리만 (재시작 시 초기화)
